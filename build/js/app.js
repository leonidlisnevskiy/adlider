...
if (window.jQuery){
	...
	$window.scroll(function(){
		...
		$sliderReview.slick({
			...
			slidesToScroll: 1,
			dots: true,
			prevArrow: '<button type="button" class="slick-prev">'+ $(this).next('li').find('.preview__title').text() +'</button>',
			nextArrow: '<button type="button" class="slick-next">' + $(this).next('li').find('.preview__title').text() + '</button>',
			responsive: [
			{
				breakpoint: 768,
				settings: {
